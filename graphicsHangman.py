...
def wordComplete(): #returns True if all letters in word have been guessed, false otherwise
    if data["incorrect"]==7:#prints false if hangman is complete
    ...
    if data["correct"]==data["number"]:#prints true if you win
        Sprite(RectangleAsset(1200,1000,LineStyle(3,Color(0xBBFFFF,1)),Color(0xBBFFFF,1)))
        Sprite(TextAsset("True",fill=Color(0xFF1493,1),style="40pt Times"),(450,150))
...
def printHangman(incorrect): #prints out new body part with each wrong guess
    if incorrect<=6:
       ...
       wordComplete()#once the hangman is complete
...
def keyPress(event): #puts letter in word if correct and  puts letter in list of guessed letters
    ...
    x1=0
    if data["letter"] not in data["word"]:
        data["incorrect"]+=1
        printHangman(data["incorrect"])
    else:
        for ch in data["word"]:#if it it correct, fills in blank
            if data["letter"]==ch:
                Sprite(TextAsset(data["letter"],fill=Color(0x000000,1),style="15pt Times"),(135+x1,325))
            x1+=30
        x1+=30
        if data["letter"] in data["word"]:
            data["correct"]+=1
            if data["correct"]==data["number"]:
                wordComplete()
